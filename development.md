...
![Общий вид дашборда](images/dash-5.jpg)

## Введение
Дашборд «Аналитика отдела клиентского сервиса за 2019 год» разработан для анализа ключевых показателей. Он позволяет отслеживать метрики, связанные с обработкой заявок, временем ответа и качеством обслуживания клиентов.
...
![Модель данных](images/data-model.PNG)
...
- **Календарь** для возможности расширения аналитики, например, добавления анализа план-факта.
...
Начальные этапы разработки уже можно увидеть в [этом ноутбуке](client-service.ipynb).

---

## Метрики

![Карта метрик](images/metric-map-7.png)
...
- **Дельта ответа (час)**: Время, прошедшее с момента регистрации заявки до ответа менеджера.
...
## Заключение
Таким образом, дашборд «Аналитика отдела клиентского сервиса за 2019 год» успешно решает задачи мониторинга и анализа ключевых показателей. В проекте реализована витрина данных в PostgreSQL, создана модель для анализа с использованием OLAP-куба и настроены визуализации в Power BI.
